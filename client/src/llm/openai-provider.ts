// openai-provider.ts — OpenAI-compatible LLM provider (browser port).
// No changes from server version: fetch is available in browser natively.
...
export class OpenAIProvider implements LLMClient {
  ...
  async call(request: LLMRequest): Promise<LLMResponse> {
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
      ...
      if (resp.status >= 400 && resp.status < 500) {
        ...
        throw new LLMError(
          `Provider error ${resp.status}: ${text}`,
          "provider_error",
        );
      }
  ...
  private _parseResponse(json: OpenAIChatCompletion): LLMResponse {
    ...
    if (!choice)
      throw new LLMError("Empty choices in response", "invalid_response");
