...
function buildPendingEvents(scenario: LoadedScenario): PendingEvent[] {
  ...
  for (const e of scenario.emails) {
    ...
    events.push({
      simTime: e.atSecond,
      fired: false,
      expand: () => [{ kind: "email", simTime: e.atSecond, email }],
    });
  }
  ...
  for (const l of scenario.logs) {
    ...
    events.push({
      simTime: l.atSecond,
      fired: false,
      expand: () => [{ kind: "log_entry", simTime: l.atSecond, entry }],
    });
  }
  ...
  for (const t of scenario.tickets) {
    ...
    events.push({
      simTime: t.atSecond,
      fired: false,
      expand: () => [{ kind: "ticket", simTime: t.atSecond, ticket }],
    });
  }

  for (const a of scenario.alarms) {
    if (a.autoFire) continue;
    ...
    events.push({
      ...
      fired: false,
      expand: () => {
        const result: ScriptedEvent[] = [
          { kind: "alarm_fired", simTime, alarm },
        ];
