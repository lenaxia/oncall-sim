 ...
 */
export function formatWallClock(simTime: number, clockAnchorMs: number): string {
  if (typeof clockAnchorMs !== 'number' || isNaN(clockAnchorMs) || clockAnchorMs === 0) {
...
// ── SimClockInput ─────────────────────────────────────────────────────────────
// The context that feeds raw server values into useSimClock.
...
export function useSimClock(): UseSimClockResult {
  const { simTime: serverSimTime, speed, paused, clockAnchorMs } = useContext(SimClockContext)

  // Sync anchor: the server sim time and the real wall time when we last received it
  const anchorRef = useRef<{ serverSimTime: number; realMs: number } | null>(null)

  // Update anchor whenever the server pushes a new simTime
  useEffect(() => {
