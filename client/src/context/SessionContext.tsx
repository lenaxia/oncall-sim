  ...
  switch (action.type) {
    ...
    case 'SSE_EVENT': {
      ...
      switch (ev.type) {
        ...
        case 'sim_time':
          return { ...state, simTime: ev.simTime, speed: ev.speed, paused: ev.paused }

        case 'chat_message': {
          const prev = state.chatMessages[ev.channel] ?? []
          return {
            ...state,
            chatMessages: { ...state.chatMessages, [ev.channel]: [...prev, ev.message] },
          }
        }
        ...
        case 'alarm_acknowledged' as SimEvent['type']:
          return {
            ...
            alarms: state.alarms.map(a =>
              ...
              a.id === (ev as any).alarmId ? { ...a, status: 'acknowledged' as const } : a
            ),
          }
        ...
        case 'ticket_comment': {
          ...
          return {
            ...state,
            ticketComments: { ...state.ticketComments, [ev.ticketId]: [...prev, ev.comment] },
          }
        }
        ...
        case 'coach_message':
          return { ...state, coachMessages: [...state.coachMessages, ev.message] }
...
export function SessionProvider({
  ...
  function dispatchAction(type: ActionType, params: Record<string, unknown> = {}): void {
    if (state.status !== 'active') return
    post(`/api/sessions/${sessionId}/actions`, { action: type, params }).catch(() => {
      onErrorRef.current(`Action failed — ${type} could not be submitted. Try again.`)
    })
  }
  ...
  async function resolveSession(): Promise<void> {
    setResolving(true)
    const res = await fetch(`/api/sessions/${sessionId}/resolve`, { method: 'POST' })
    if (!res.ok) {
      setResolving(false)
