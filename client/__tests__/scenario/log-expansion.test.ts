// log-expansion.test.ts — browser port.
// Uses loadScenarioFromText() + yaml.dump to replace fs + temp dirs.
...
} from "../../src/scenario/log-profiles";
...
async function loadWithOverrides(overrides: Record<string, unknown>) {
  const base = yaml.load(fixtureYaml) as Record<string, unknown>;
  Object.assign(base, overrides);
  return loadScenarioFromText(yaml.dump(base), noopResolve);
}
...
describe("log_patterns — basic expansion", () => {
  it("expands a pattern into the correct number of entries", async () => {
    ...
    if (!isScenarioLoadError(result)) {
      expect(result.logs.filter((l) => l.id.startsWith("pat-a")).length).toBe(
        7,
      );
    }
  });
  ...
  it("respects count cap", async () => {
    ...
    expect(isScenarioLoadError(result)).toBe(false);
    if (!isScenarioLoadError(result)) {
      expect(result.logs.filter((l) => l.id.startsWith("pat-b")).length).toBe(
        4,
      );
    }
  });
...
describe("log_patterns — jitter", () => {
  it("without jitter all timestamps are on exact interval multiples", async () => {
    ...
    expect(isScenarioLoadError(result)).toBe(false);
    if (!isScenarioLoadError(result)) {
      const times = result.logs
        .filter((l) => l.id.startsWith("pat-exact"))
        .map((e) => e.atSecond);
  ...
  it("with jitter timestamps are NOT all on exact multiples (statistical)", async () => {
    ...
    if (!isScenarioLoadError(result)) {
      ...
      const allExact = pat.every((e, i) => e.atSecond === i * 10);
  ...
  it("with jitter all entries still stay within [from_second, to_second]", async () => {
    ...
    expect(isScenarioLoadError(result)).toBe(false);
    if (!isScenarioLoadError(result)) {
      for (const e of result.logs.filter((l) => l.id.startsWith("pat-j2"))) {
  ...
  it("seeded jitter is deterministic across two loads", async () => {
    ...
    if (!isScenarioLoadError(r1) && !isScenarioLoadError(r2)) {
      const a = r1.logs
        .filter((l) => l.id.startsWith("pat-seed"))
        .map((l) => l.atSecond);
      const b = r2.logs
        .filter((l) => l.id.startsWith("pat-seed"))
        .map((l) => l.atSecond);
  ...
  it("unseeded jitter produces different timestamps on two loads (statistical)", async () => {
    ...
    if (!isScenarioLoadError(r1) && !isScenarioLoadError(r2)) {
      const a = r1.logs
        .filter((l) => l.id.startsWith("pat-unseeded"))
        .map((l) => l.atSecond);
      const b = r2.logs
        .filter((l) => l.id.startsWith("pat-unseeded"))
        .map((l) => l.atSecond);
...
describe("background_logs — basic expansion", () => {
  it("expands java_web_service profile into a non-empty log list", async () => {
    ...
    if (!isScenarioLoadError(result)) {
      expect(
        result.logs.filter((l) => l.id.startsWith("bg-0")).length,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it("all background entries have atSecond inside [from_second, to_second]", async () => {
    ...
    expect(isScenarioLoadError(result)).toBe(false);
    if (!isScenarioLoadError(result)) {
      for (const e of result.logs.filter((l) => l.id.startsWith("bg-0"))) {
        expect(e.atSecond).toBeGreaterThanOrEqual(10);
  ...
  it("high density produces more entries than low density", async () => {
    ...
    const rHigh = await loadWithOverrides({
      background_logs: [{ ...base, density: "high", seed: 5 }],
    });
    const rLow = await loadWithOverrides({
      background_logs: [{ ...base, density: "low", seed: 5 }],
    });
    ...
    if (!isScenarioLoadError(rHigh) && !isScenarioLoadError(rLow)) {
      expect(
        rHigh.logs.filter((l) => l.id.startsWith("bg-0")).length,
      ).toBeGreaterThan(
        rLow.logs.filter((l) => l.id.startsWith("bg-0")).length,
      );
    }
  });

  it("entries only use messages defined in the profile", async () => {
    const profileLines = new Set(
      LOG_PROFILES["java_web_service"].lines.map((l) => l.message),
    );
    const result = await loadWithOverrides({
      background_logs: [
        {
...
describe("background_logs — seeded vs unseeded RNG", () => {
  ...
  it("unseeded background produces different streams on two loads (statistical)", async () => {
    ...
    if (!isScenarioLoadError(r1) && !isScenarioLoadError(r2)) {
      expect(
        r1.logs.filter((l) => l.id.startsWith("bg-0")).map((e) => e.atSecond),
      ).not.toEqual(
        r2.logs.filter((l) => l.id.startsWith("bg-0")).map((e) => e.atSecond),
      );
    }
  });
});
...
describe("getDensityMultiplier", () => {
  it("low < medium < high", () => {
    expect(getDensityMultiplier("low")).toBeLessThan(
      getDensityMultiplier("medium"),
    );
    expect(getDensityMultiplier("medium")).toBeLessThan(
      getDensityMultiplier("high"),
    );
  });
