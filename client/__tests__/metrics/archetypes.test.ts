...
import { getArchetypeDefaults, getValidArchetypes } from '../../src/metrics/archetypes'

describe('getArchetypeDefaults', () => {
  it('returns defaults for all HLD §8.4 archetypes', () => {
    const expected = [
      'request_rate', 'error_rate', 'fault_rate', 'availability', 'throughput_bytes',
      'p50_latency_ms', 'p99_latency_ms', 'p999_latency_ms',
      'cpu_utilization', 'memory_heap', 'memory_jvm', 'memory_system', 'thread_count',
      'disk_usage', 'disk_iops', 'network_in_bytes', 'network_out_bytes',
      'connection_pool_used', 'queue_depth', 'queue_age_ms',
  ...
  it('throws for unknown archetype', () => {
    expect(() => getArchetypeDefaults('nonexistent_metric')).toThrow(/Unknown archetype/)
  })

  it('error_rate has sporadic_spikes noise type', () => {
    expect(getArchetypeDefaults('error_rate').noiseType).toBe('sporadic_spikes')
  })
  ...
  it('cpu_utilization has random_walk noise type', () => {
    expect(getArchetypeDefaults('cpu_utilization').noiseType).toBe('random_walk')
  })
  ...
  it('percentage archetypes have maxValue of 100', () => {
    ['error_rate', 'fault_rate', 'availability', 'cpu_utilization',
     'disk_usage', 'conversion_rate'].forEach(a => {
...
describe('getValidArchetypes', () => {
  ...
  it('includes all HLD §8.4 archetypes', () => {
    ...
    const required = [
      ...
      'disk_usage', 'disk_iops', 'network_in_bytes', 'network_out_bytes',
      'connection_pool_used', 'queue_depth', 'queue_age_ms',
      'conversion_rate', 'active_users', 'cert_expiry', 'custom',
    ]
