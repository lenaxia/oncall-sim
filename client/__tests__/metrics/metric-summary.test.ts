...
describe("MetricNarrative — status band classification", () => {
  it("returns healthy when value is below warning threshold", async () => {
  ...
  it("returns warning when value is between warning and critical", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
      flatSeries(-60, 60, 5),
  ...
  it("returns critical when value is at or above critical threshold", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
      flatSeries(-60, 60, 90),
  ...
  it("returns unknown when store has no data", async () => {
...
describe("MetricNarrative — slope derivation", () => {
  it("returns stable for a flat series", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
  ...
  it("returns rising sharply for a steeply rising series", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
  ...
  it("returns recovering for a steeply falling series", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
  ...
  it("returns unknown when fewer than 3 points in the window", async () => {
...
describe("MetricNarrative — pre-incident value", () => {
  it("captures the value just before onsetSecond", async () => {
  ...
  it("preIncident is null when there is no data before onsetSecond", async () => {
...
describe("MetricNarrative — timeInBand", () => {
  it("is 0 for a metric that just crossed into critical", async () => {
  ...
  it("is large for a metric that has been critical for a long time", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
...
describe("MetricNarrative — sentence content", () => {
  it("healthy+stable sentence does not mention rising or critical", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
  ...
  it('critical+rising sentence contains "critical" and direction language', async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
  ...
  it('critical+stable sentence for saturated archetype mentions "saturated"', async () => {
    const store = storeFromSeries(
      "payment-service",
  ...
  it('recovering sentence says "recovered" or "improving"', async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
  ...
  it("sentence includes pre-incident value when available", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
      rampSeries(-60, 0, 120, 0.3, 90),
...
describe("renderMetricSummary", () => {
  ...
  it("returns empty string when no narratives", async () => {
    const summary = { simTime: 0, narratives: [] };
    const scenario = await makeScenario();
    expect(renderMetricSummary(summary, scenario)).toBe("");
  });

  it("output contains the grounding instruction", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
      flatSeries(-60, 60, 90),
  ...
  it("focal service is labelled as (focal service)", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
      flatSeries(-60, 60, 90),
  ...
  it("exonerated service gets a single short line", async () => {
  ...
  it("independent service shows value+status but no sentence prose", async () => {
    const store = createMetricStore(
      {
  ...
  it("full output contains metric sentence for focal service", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
  ...
  it("output contains the LLM grounding rules", async () => {
    const store = storeFromSeries(
      "payment-service",
      "error_rate",
      flatSeries(-60, 60, 90),
...
describe("computeMetricSummary — integration with fixture scenario", () => {
  ...
  it("loads without error and returns a narrative per focal metric", async () => {
    const scenario = await await getFixtureScenario();
  ...
  it("simTime is preserved in the summary", async () => {
    const scenario = await await getFixtureScenario();
    const nullStore: MetricStore = {
      getAllSeries: () => ({}),
      getCurrentValue: () => null,
