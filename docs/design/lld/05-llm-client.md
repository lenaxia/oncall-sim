...
tick(context):
  ...
  4. For each tool call in response:
     ...
     b. Validate against scenario llm_event_tools constraints:
        ...
        - trigger_metric_recovery: Phase 2 — reject with log if called
     c. Execute valid tool calls:
        ...
        - trigger_cascade → fires alarm_fired + inject_log_entry for the cascading service
                            (metric effects are Phase 2 only)
        - trigger_metric_spike → Phase 2 only — rejected by validateToolCall, never reaches here
...
- `trigger_metric_recovery`: always rejected — Phase 2 only
- `trigger_metric_spike`: always rejected — Phase 2 only
...
export function buildMockLLMProvider(responses: MockLLMResponses): MockProvider {
  return new MockProvider(responses)
}
```
...
getStakeholderTools:
  ...
  - trigger_metric_recovery never included (Phase 2)
...
validateToolCall:
  ...
  - trigger_metric_recovery → always valid=false with 'Phase 2 only' reason
...
tick — error paths:
  ...
  - trigger_metric_recovery in response → skipped with log, does not crash
...
- [ ] `tool-definitions.ts` defines all stakeholder and coach tools
- [ ] `validateToolCall` rejects invalid calls without throwing
- [ ] `StakeholderEngine.tick` never throws — all errors logged and swallowed
- [ ] `trigger_metric_recovery` always rejected (Phase 2)
