...
// Minimal MetricStore stub for tests that don't exercise reactive overlays
function nullMetricStore(): MetricStore {
  return {
    getAllSeries: () => ({}),
...
describe("StakeholderEngine.tick — error paths", () => {
  it("LLMError thrown → returns [] (never throws)", async () => {
    const throwingClient = {
      call: (): Promise<never> => {
        throw new LLMError("test error", "provider_error");
      },
    };
    const engine = createStakeholderEngine(
      throwingClient,
