...
import {
  getFixtureScenario, clearFixtureCache, buildTestClock,
  expectEvent, expectNoEvent, expectAction,
...
function scenario() { return getFixtureScenario() }

function makeDeps(overrides: Partial<GameLoopDependencies> = {}): GameLoopDependencies {
...
describe('GameLoop — tick sequence (timer-driven)', () => {
  it('scripted event at t=0 fires on first tick via game loop', () => {
    vi.useFakeTimers()
    try {
      ...
      vi.advanceTimersByTime(scenario().engine.tickIntervalSeconds * 1000)
      loop.stop()
  ...
  it('scripted event at t=30 does not fire until simTime >= 30', () => {
  ...
  it('onDirtyTick called after scripted event fires (via timer tick)', () => {
    vi.useFakeTimers()
    try {
      const dirtyTick = vi.fn().mockResolvedValue([])
      const deps = makeDeps({ onDirtyTick: dirtyTick })
  ...
  it('inFlight=true prevents second concurrent onDirtyTick call', async () => {
    ...
    const inflightPromise = new Promise<SimEvent[]>(res => { resolveTick = res })
    let callCount = 0
  ...
  it('inFlight clears after onDirtyTick resolves', async () => {
    let resolveTick!: (v: SimEvent[]) => void
    const inflightPromise = new Promise<SimEvent[]>(res => { resolveTick = res })
    const dirtyTick = vi.fn().mockReturnValueOnce(inflightPromise).mockResolvedValue([])
    const deps = makeDeps({ onDirtyTick: dirtyTick })
    const loop = createGameLoop(deps)
...
describe('GameLoop — handleAction', () => {
  it('action recorded in auditLog', () => {
    const deps = makeDeps()
    const loop = createGameLoop(deps)
  ...
  it('correct SimEvent emitted via onEvent after handleAction', () => {
    const deps   = makeDeps()
    const loop   = createGameLoop(deps)
    const events: SimEvent[] = []
    loop.onEvent(e => events.push(e))
  ...
  it('session marked dirty after action — onDirtyTick is called', () => {
    const dirtyTick = vi.fn().mockResolvedValue([])
  ...
  it('onDirtyTick called immediately if not in-flight', () => {
    const dirtyTick = vi.fn().mockResolvedValue([])
    const loop = createGameLoop(makeDeps({ onDirtyTick: dirtyTick }))
    loop.handleAction('open_tab', {})
    expect(dirtyTick).toHaveBeenCalledTimes(1)
  ...
  it('update_ticket action emits ticket_updated event', () => {
    const deps   = makeDeps()
    const loop   = createGameLoop(deps)
    const events: SimEvent[] = []
    loop.onEvent(e => events.push(e))
  ...
  it('suppress_alarm action emits alarm_silenced event', () => {
    const deps   = makeDeps()
    const loop   = createGameLoop(deps)
    const events: SimEvent[] = []
    loop.onEvent(e => events.push(e))
    deps.store.addAlarm({ id: 'a1', service: 'svc', metricId: 'error_rate', condition: '>5', value: 10, severity: 'SEV2', status: 'firing', simTime: 0 })
...
describe('GameLoop — getSnapshot', () => {
  it('returns SessionSnapshot with correct sessionId and scenarioId', () => {
    const deps  = makeDeps()
    const loop  = createGameLoop(deps)
  ...
  it('metrics field matches pre-generated metrics', () => {
    const deps = makeDeps()
    const loop = createGameLoop(deps)
    const snap = loop.getSnapshot()
    expect(snap.metrics).toBe(deps.metrics)
  })

  it('simTime reflects clock state', () => {
    const deps  = makeDeps()
  ...
  it('coachMessages starts empty in Phase 4', () => {
    const loop = createGameLoop(makeDeps())
    expect(loop.getSnapshot().coachMessages).toEqual([])
  })

  it('handleCoachMessage appends to coachMessages in snapshot', () => {
    const loop = createGameLoop(makeDeps())
    loop.handleCoachMessage({ id: 'c1', text: 'Check deployments', simTime: 30, proactive: true })
...
describe('GameLoop — pause/resume/setSpeed', () => {
  it('pause() stops sim time advancing', () => {
    const deps  = makeDeps()
    const clock = deps.clock as ReturnType<typeof buildTestClock>
    const loop  = createGameLoop(deps)
    loop.pause()
  ...
  it('resume() resumes sim time advancing', () => {
    const deps  = makeDeps()
    const clock = deps.clock as ReturnType<typeof buildTestClock>
    const loop  = createGameLoop(deps)
    loop.pause()
  ...
  it('setSpeed changes speed reflected in snapshot', () => {
    const loop = createGameLoop(makeDeps())
    loop.setSpeed(10)
    expect(loop.getSnapshot().speed).toBe(10)
  })

  it('setSpeed changes tick rate — clock advances faster with higher speed', () => {
    const deps  = makeDeps()
    const clock = deps.clock as ReturnType<typeof buildTestClock>
    const loop  = createGameLoop(deps)
    loop.setSpeed(5)
...
describe('GameLoop — getEvaluationState', () => {
  ...
  it('reflects actions taken via handleAction', () => {
    const deps = makeDeps()
    const loop = createGameLoop(deps)
...
describe('GameLoop — onEvent', () => {
  ...
  it('no events emitted before any action', () => {
    const events: SimEvent[] = []
    const loop = createGameLoop(makeDeps())
    loop.onEvent(e => events.push(e))
...
describe('GameLoop — stakeholder events update conversation store', () => {
  it('chat_message from onDirtyTick appears in getSnapshot().chatChannels', async () => {
    const chatEvent: SimEvent = {
      type:    'chat_message',
      channel: '#incidents',
      message: { id: 'msg-1', channel: '#incidents', persona: 'fixture-persona', text: 'Investigating', simTime: 10 },
    }
    const dirtyTick = vi.fn().mockResolvedValueOnce([chatEvent]).mockResolvedValue([])
    const deps = makeDeps({ onDirtyTick: dirtyTick })
    const loop = createGameLoop(deps)
  ...
  it('alarm_fired from onDirtyTick appears in getSnapshot().alarms', async () => {
    const alarmEvent: SimEvent = {
      type:  'alarm_fired',
      alarm: { id: 'dyn-alarm', service: 'svc', metricId: 'error_rate', condition: '>5', value: 10, severity: 'SEV2', status: 'firing', simTime: 10 },
    }
    const dirtyTick = vi.fn().mockResolvedValueOnce([alarmEvent]).mockResolvedValue([])
    const deps = makeDeps({ onDirtyTick: dirtyTick })
    const loop = createGameLoop(deps)
