...
describe("GameLoop — metric_update SSE streaming", () => {
  it("emits metric_update events when MetricStore has reactive overlay points in window", () => {
    vi.useFakeTimers();
    try {
  ...
  it("getSnapshot returns MetricStore series when metricStore is provided", () => {
    const s = getFixtureScenario();
