...
describe('ScenarioSchema — valid fixture', () => {
  ...
  it('logs field is required — omitting it fails', () => {
    const raw = loadFixture() as Record<string, unknown>
  ...
  it('remediation_actions field is required — omitting it fails', () => {
    const raw = loadFixture() as Record<string, unknown>
    const result = ScenarioSchema.safeParse({ ...raw, remediation_actions: undefined })
    expect(result.success).toBe(false)
  })
...
describe('ScenarioSchema — field constraints', () => {
  ...
  it('rejects invalid service_type', () => {
    const result = ScenarioSchema.safeParse(withOverride({ service_type: 'cache' }))
    expect(result.success).toBe(false)
  })

  it('rejects invalid difficulty', () => {
    const result = ScenarioSchema.safeParse(withOverride({ difficulty: 'expert' }))
    expect(result.success).toBe(false)
  })
  ...
  it('rejects invalid correlation type', () => {
    ...
    const result = ScenarioSchema.safeParse({
      ...
      ops_dashboard: {
        ...ops,
        correlated_services: [{ name: 'svc', correlation: 'unknown', health: 'healthy' }],
      },
    })
    expect(result.success).toBe(false)
  })
...
describe('ScenarioSchema — cross-reference data available after parse', () => {
  ...
  it('persona ids are present for cross-ref validation', () => {
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data.personas.every(p => typeof p.id === 'string')).toBe(true)
    }
  })
  ...
  it('remediation_actions ids are present for cross-ref validation', () => {
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data.remediation_actions.every(r => typeof r.id === 'string')).toBe(true)
    }
  })
