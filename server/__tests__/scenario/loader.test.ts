...
describe('loadScenario — happy paths', () => {
  ...
  it('LoadedScenario has no metrics field — metrics are session-scoped', async () => {
    const result = await loadScenario(getFixtureScenarioDir())
    if (!isScenarioLoadError(result)) {
      expect((result as unknown as Record<string, unknown>)['metrics']).toBeUndefined()
    }
  })
  ...
  it('camelCase transformation is applied — service_type → serviceType', async () => {
    ...
    if (!isScenarioLoadError(result)) {
      ...
      expect((result as unknown as Record<string, unknown>)['service_type']).toBeUndefined()
    }
  })
  ...
  it('opsDashboard.focalService has correct name and trafficProfile', async () => {
    ...
    if (!isScenarioLoadError(result)) {
      ...
      expect(result.opsDashboard.focalService.trafficProfile).toBe('always_on_api')
    }
  })
})
...
describe('loadScenario — ops_dashboard_file', () => {
  ...
  beforeAll(async () => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'scenario.yaml'),
      (await import('js-yaml')).dump(fixtureObj))
    fs.writeFileSync(path.join(tmpDir, 'metrics.yaml'),
...
describe('loadScenario — error paths', () => {
  ...
  it('Zod schema failure returns ScenarioLoadError with field paths', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oncall-test-'))
    fs.writeFileSync(path.join(tmpDir, 'scenario.yaml'), 'id: missing-required-fields\n')
    const result = await loadScenario(tmpDir)
    expect(isScenarioLoadError(result)).toBe(true)
    if (isScenarioLoadError(result)) {
  ...
  it('ops_dashboard + ops_dashboard_file both present returns error', async () => {
    ...
    expect(isScenarioLoadError(result)).toBe(true)
    if (isScenarioLoadError(result)) {
      expect(result.errors.find(e => e.field === 'ops_dashboard_file')).toBeDefined()
    }
  })
  ...
  it('path traversal in file reference returns error', async () => {
    ...
    const fixtureSrc = fs.readFileSync(path.join(fixtureDir, 'scenario.yaml'), 'utf8')
    const fixtureObj = (await import('js-yaml')).load(fixtureSrc) as Record<string, unknown>
    const wiki = fixtureObj['wiki'] as { pages: Array<Record<string, unknown>> }
    wiki.pages[0] = { ...wiki.pages[0], content: undefined, content_file: '../../etc/passwd' }
    ...
    expect(isScenarioLoadError(result)).toBe(true)
    if (isScenarioLoadError(result)) {
      expect(result.errors.find(e => e.message.includes('traversal'))).toBeDefined()
    }
  })
  ...
  it('missing referenced file returns error', async () => {
    ...
    const fixtureObj = (await import('js-yaml')).load(fixtureSrc) as Record<string, unknown>
    const wiki = fixtureObj['wiki'] as { pages: Array<Record<string, unknown>> }
    wiki.pages[0] = { ...wiki.pages[0], content: undefined, content_file: 'ghost-file.md' }
    ...
    expect(isScenarioLoadError(result)).toBe(true)
    if (isScenarioLoadError(result)) {
      expect(result.errors.find(e => e.message.includes('ghost-file.md'))).toBeDefined()
    }
  })
})
...
describe('loadAllScenarios', () => {
  ...
  it('valid scenario added to map, invalid excluded', async () => {
    ...
    const fixtureSrc = fs.readFileSync(
      path.join(getFixtureScenarioDir(), 'scenario.yaml'), 'utf8'
    )
    const fixtureObj = (await import('js-yaml')).load(fixtureSrc) as Record<string, unknown>
    fixtureObj['id'] = 'valid-scenario'
    fs.writeFileSync(path.join(validDir, 'scenario.yaml'),
      (await import('js-yaml')).dump(fixtureObj))
...
describe('toScenarioSummary', () => {
  it('returns only picker-screen fields', async () => {
    ...
    if (!isScenarioLoadError(result)) {
      ...
      expect(Object.keys(summary).sort()).toEqual(
        ['description', 'difficulty', 'id', 'serviceType', 'tags', 'title']
      )
  ...
  it('summary does not include personas, alarms, or opsDashboard', async () => {
    ...
    if (!isScenarioLoadError(result)) {
      const summary = toScenarioSummary(result)
      expect((summary as unknown as Record<string, unknown>)['personas']).toBeUndefined()
      expect((summary as unknown as Record<string, unknown>)['alarms']).toBeUndefined()
      expect((summary as unknown as Record<string, unknown>)['opsDashboard']).toBeUndefined()
    }
  })
})
