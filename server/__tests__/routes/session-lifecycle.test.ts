...
beforeAll(async () => {
  ...
  if (isScenarioLoadError(result)) throw new Error('Failed to load fixture: ' + JSON.stringify(result.errors))
...
describe('POST /api/sessions/:id/resolve', () => {
  ...
  it('GET /api/sessions/:id/debrief returns 200 after resolve', async () => {
    ...
    const debrief = await request(app).get(`/api/sessions/${sessionId}/debrief`)
  ...
  it('resolve already-resolved session → 409', async () => {
    ...
    await request(app).post(`/api/sessions/${sessionId}/resolve`)
    const second = await request(app).post(`/api/sessions/${sessionId}/resolve`)
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(
      ...
      res => {
        ...
        res.on('close', () => { clearTimeout(timer); resolve(collected) })
      }
    )
...
describe('GET /api/sessions/:id/events', () => {
  ...
  it('first event is session_snapshot', async () => {
    const create = await request(app).post('/api/sessions').send({ scenarioId: '_fixture' })
    const { sessionId } = create.body as { sessionId: string }

    const events = await readSSEUntil(
      server, sessionId,
      evs => evs.some(e => e.type === 'session_snapshot')
    )
  ...
  it('session_expired sent when connecting to unknown session', async () => {
    const events = await readSSEUntil(
      server, 'nonexistent-session-id',
      evs => evs.some(e => e.type === 'session_expired')
    )
  ...
  it('session_expired sent on reconnect after session expiry', async () => {
    ...
    const shortApp = createApp(new Map([['_fixture', result]]), shortExpiryStore, shortBroker, createLLMClient())
...
describe('SSE event delivery — chat and email', () => {
  ...
  it('chat_message SSE event broadcast with trainee message', async () => {
    ...
    const eventsPromise = readSSEUntil(
      server, sessionId,
      evs => evs.some(e => e.type === 'chat_message' &&
        e.type === 'chat_message' && e.message.persona === 'trainee'),
      2000
    )
    ...
    const events = await eventsPromise
    const chatEv = events.find(e => e.type === 'chat_message' && e.message.persona === 'trainee')
  ...
  it('email_received SSE event broadcast with trainee reply', async () => {
    ...
    const eventsPromise = readSSEUntil(
      server, sessionId,
      evs => evs.some(e => e.type === 'email_received' &&
        e.type === 'email_received' && e.email.from === 'trainee'),
      2000
    )
    ...
    const events = await eventsPromise
    const emailEv = events.find(e => e.type === 'email_received' && e.email.from === 'trainee')
