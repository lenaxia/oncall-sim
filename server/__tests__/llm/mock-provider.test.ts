...
import { loadMockResponses } from '../../src/llm/mock-provider'
import type { MockLLMResponses } from '../../src/llm/mock-provider'
import { getFixtureScenarioDir, getMockLLMProvider, buildMockLLMProvider } from '../../src/testutil/index'
...
describe('loadMockResponses', () => {
  it('loads the fixture mock-llm-responses.yaml without error', () => {
    const responses = loadMockResponses(getFixtureScenarioDir())
    expect(responses.stakeholder_responses).toBeDefined()
    expect(responses.coach_responses).toBeDefined()
    expect(responses.debrief_response).toBeDefined()
  })

  it('fixture has at least one stakeholder response', () => {
    const responses = loadMockResponses(getFixtureScenarioDir())
    expect(responses.stakeholder_responses.length).toBeGreaterThan(0)
  })
})
...
describe('MockProvider.call — stakeholder role', () => {
  it('tick_1 trigger returns matching tool_calls', async () => {
    const provider = buildMockLLMProvider(makeResponses({
      stakeholder_responses: [{
        trigger:    'tick_1',
        tool_calls: [{ tool: 'send_message', params: { persona: 'p1', channel: '#inc', message: 'hi' } }],
      }],
    }))
    const resp = await provider.call({
      role: 'stakeholder', messages: [], tools: [], sessionId: 'sess',
    })
  ...
  it('tick_99 (no match) returns empty response', async () => {
    const provider = buildMockLLMProvider(makeResponses())
    // Advance past tick_1 by calling 98 times
    for (let i = 0; i < 98; i++) {
      await provider.call({ role: 'stakeholder', messages: [], tools: [], sessionId: 'sess' })
    }
    const resp = await provider.call({ role: 'stakeholder', messages: [], tools: [], sessionId: 'sess' })
  ...
  it('after_action trigger matches when user message contains action', async () => {
    const provider = buildMockLLMProvider(makeResponses({
      stakeholder_responses: [{
        ...
        tool_calls: [{ tool: 'inject_log_entry', params: { service: 'svc', level: 'INFO', message: 'recovering' } }],
      }],
    }))
    const resp = await provider.call({
      role:     'stakeholder',
      messages: [{ role: 'user', content: 'action: trigger_rollback service: fixture-service' }],
      tools:    [],
      sessionId: 'sess',
    })
    expect(resp.toolCalls.length).toBe(1)
  ...
  it('after_action with wrong param: no match → empty response', async () => {
    ...
    const resp = await provider.call({
      role:     'stakeholder',
      messages: [{ role: 'user', content: 'action: trigger_rollback service: service-B' }],
      tools:    [],
      sessionId: 'sess',
    })
    expect(resp.toolCalls).toHaveLength(0)
  ...
  it('empty responses file: all calls return empty response', async () => {
    const provider = buildMockLLMProvider(makeResponses())
    const resp = await provider.call({ role: 'stakeholder', messages: [], tools: [], sessionId: 's' })
    expect(resp.toolCalls).toHaveLength(0)
  })
})
...
describe('MockProvider.call — coach role', () => {
  it('on_demand trigger returns matching message', async () => {
    const provider = buildMockLLMProvider(makeResponses({
      coach_responses: [{ trigger: 'on_demand', message: 'Check deployments.' }],
    }))
    const resp = await provider.call({
      role:     'coach',
  ...
  it('proactive_tick_2 returns matching message', async () => {
    const provider = buildMockLLMProvider(makeResponses({
      coach_responses: [{ trigger: 'proactive_tick_2', message: 'Have you checked logs?' }],
    }))
    // First proactive tick
    await provider.call({ role: 'coach', messages: [], tools: [], sessionId: 'sess' })
    // Second proactive tick should match
    const resp = await provider.call({ role: 'coach', messages: [], tools: [], sessionId: 'sess' })
...
describe('MockProvider.call — debrief role', () => {
  it('debrief role returns narrative text', async () => {
    const provider = buildMockLLMProvider(makeResponses({
      debrief_response: { narrative: 'You identified the bad deploy and rolled back.' },
    }))
    const resp = await provider.call({ role: 'debrief', messages: [], tools: [], sessionId: 'sess' })
...
describe('testutil — getMockLLMProvider / buildMockLLMProvider', () => {
  it('getMockLLMProvider returns a working MockProvider', async () => {
    ...
    const resp = await provider.call({
      role: 'stakeholder', messages: [], tools: [], sessionId: 'test',
    })
