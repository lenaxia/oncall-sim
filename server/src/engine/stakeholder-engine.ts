...
import type { SimEvent, ChatMessage, EmailMessage, LogEntry, Alarm, TicketComment } from '@shared/types/events'
...
export function createStakeholderEngine(
  ...
  async function _tick(context: StakeholderContext): Promise<SimEvent[]> {
    ...
    const activeAlarmIds = new Set(context.conversations.alarms.map(a => a.id))
  ...
  function _buildPrompt(context: StakeholderContext, eligiblePersonaIds: string[]): LLMMessage[] {
    const eligiblePersonas = scenario.personas.filter(p => eligiblePersonaIds.includes(p.id))
    ...
    for (const entry of context.auditLog) {
      auditLines.push(`[t=${entry.simTime}] ${entry.action} ${JSON.stringify(entry.params)}`)
    }
   ...
   *
   * Strategy (LLD 05 §4):
   * - Audit log and cooldowns are always preserved (small, critical).
   * - Chat messages and emails older than the recent window are summarized as a
   *   single truncated prefix rather than dropped silently.
   * - Most-recent messages are always kept.
   * - Best-effort: actual provider tokenisation may differ, but this prevents
   *   runaway context growth on long scenarios.
   */
  function _buildUserContent(
    ...
    const chatLines = allChats.map(
      msg => `[t=${msg.simTime}] ${msg.channel} | ${msg.persona}: ${msg.text}`
    )

    const emails = [...context.conversations.emails].sort((a, b) => a.simTime - b.simTime)
    const emailLines = emails.map(
      e => `[t=${e.simTime}] EMAIL | from:${e.from} to:${e.to} | ${e.subject}: ${e.body.slice(0, 200)}`
    )
    ...
    const fullConversation = ['## Conversation History', ...allHistoryLines].join('\n')
    ...
    conversationSection.push(...allHistoryLines.slice(allHistoryLines.length - kept))
  ...
  function _executeTool(
    ...
    switch (tool) {
      ...
      case 'add_ticket_comment': {
        ...
        return [{ type: 'ticket_comment', ticketId: comment.ticketId, comment }]
      }
