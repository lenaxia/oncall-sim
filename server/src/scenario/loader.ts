  ...
  for (const dir of subdirs) {
    ...
    if (isScenarioLoadError(result)) {
      log.error(
        ...
        "Scenario failed validation — excluded",
      );
    } else {
      const metricCount = Object.values(
        result.opsDashboard.focalService.metrics,
      ).length;
      log.info(
        { scenarioId: result.id, metrics: metricCount },
        "Scenario loaded",
      );
...
/**
 * Expands a single log_pattern into ScriptedLogEntry objects.
 * Entries are placed at nominal_time ± jitter, clamped inside [from_second, to_second].
 * When seed is absent, Math.random() is used — so each session differs.
 */
...
/**
 * Expands a single background_logs block into ScriptedLogEntry objects by
 * sampling the named profile's lines using weighted random selection and
 * scattering them across [from_second, to_second] at randomised intervals.
 * When seed is absent, Math.random() is used — so each session differs.
 */
function expandBackgroundLogs(
  ...
  // Build a weighted table for line selection
...
async function transform(
  ...
  // Logs — merge scripted entries, pattern expansions, and background noise
  ...
  const cicd: CICDConfig = {
    pipelines: (raw.cicd.pipelines ?? []).map((p) => ({
      ...
      service: p.service,
      stages: (p.stages ?? []).map(
        (s): import("../scenario/types").PipelineStageConfig => ({
          id: s.id,
          name: s.name,
          type: s.type,
