...
export function getFixtureScenario(): LoadedScenario {
  ...
  const raw = yaml.load(fs.readFileSync(yamlPath, "utf8")) as Record<
    string,
    unknown
  >;
...
function rawToLoadedScenario(raw: Record<string, unknown>): LoadedScenario {
  ...
  return {
    ...
    engine: {
      ...
      defaultTab: ((engine.default_tab as string | undefined) ??
        "email") as import("../scenario/types").TabId,
      llmEventTools: (
        (engine.llm_event_tools ?? []) as Array<Record<string, unknown>>
      ).map((t) => ({
    ...
    personas: ((raw.personas ?? []) as Array<Record<string, unknown>>).map(
      (p) => ({
        id: p.id as string,
    ...
    remediationActions: (
      (raw.remediation_actions ?? []) as Array<Record<string, unknown>>
    ).map((r) => ({
      id: r.id as string,
    ...
    evaluation: (() => {
      ...
      return {
        ...
        relevantActions: (
          e.relevant_actions as Array<Record<string, unknown>>
        ).map((a) => ({
        ...
        redHerrings: (e.red_herrings as Array<Record<string, unknown>>).map(
          (a) => ({
            action: a.action as string,
            why: a.why as string,
          }),
    ...
    chat: (() => {
      ...
      return {
        channels: ((c.channels ?? []) as Array<Record<string, unknown>>).map(
          (ch) => ({
            id: ch.id as string,
            name: ch.name as string,
          }),
        ),
        messages: ((c.messages ?? []) as Array<Record<string, unknown>>).map(
          (m) => ({
            id: m.id as string,
    ...
    tickets: ((raw.ticketing ?? []) as Array<Record<string, unknown>>).map(
      (t) => ({
        id: t.id as string,
    ...
    opsDashboard: {
      ...
      focalService: {
        ...
        metrics: (
          (focalRaw.metrics ?? []) as Array<Record<string, unknown>>
        ).map(transformMetric),
      },
      correlatedServices: (
        (ops.correlated_services ?? []) as Array<Record<string, unknown>>
      ).map((cs) => ({
        name: cs.name as string,
        correlation: cs.correlation as
          | "upstream_impact"
          | "exonerated"
          | "independent",
        ...
        overrides: ((cs.overrides ?? []) as Array<Record<string, unknown>>).map(
          transformMetric,
        ),
      })),
    },
    ...
    cicd: (() => {
      ...
      return {
        pipelines: ((c.pipelines ?? []) as Array<Record<string, unknown>>).map(
          (p) => ({
            id: p.id as string,
            name: p.name as string,
            service: p.service as string,
            stages: ((p.stages ?? []) as Array<Record<string, unknown>>).map(
              (s) => ({
                id: s.id as string,
                ...
                previousVersion:
                  (s.previous_version as string | null | undefined) ?? null,
                status:
                  s.status as import("../scenario/types").PipelineStageConfig["status"],
                deployedAtSec: s.deployed_at_sec as number,
                commitMessage: s.commit_message as string,
                author: s.author as string,
                blockers: (
                  (s.blockers ?? []) as Array<Record<string, unknown>>
                ).map((b) => ({
                ...
                tests: ((s.tests ?? []) as Array<Record<string, unknown>>).map(
                  (t) => ({
                    name: t.name as string,
                    status:
                      t.status as import("@shared/types/events").TestStatus,
                    url: t.url as string | undefined,
                    note: t.note as string | undefined,
                  }),
                ),
                promotionEvents: (
                  (s.promotion_events ?? []) as Array<Record<string, unknown>>
                ).map((e) => ({
                  version: e.version as string,
                  simTime: e.sim_time as number,
                  status: e.status as "succeeded" | "failed" | "blocked",
        ...
        deployments: (
          (c.deployments ?? []) as Array<Record<string, unknown>>
        ).map((d) => ({
  ...
  return {
    ...
    incidentResponse: ir
      ? {
          ...
          saturationDurationSeconds: ir.saturation_duration_seconds as
            | number
            | undefined,
        }
      : undefined,
    seriesOverride: m.series_override as
...
export function buildTestSession(overrides: Partial<Session> = {}): Session {
  ...
  const gameLoop =
    ...
    createGameLoop({
      ...
      store: createCS(),
      evaluator: createEV(),
      metrics: generateAllMetrics(scenario, sessionId).series,
      clockAnchorMs: 0,
    });
  return {
    id: sessionId,
